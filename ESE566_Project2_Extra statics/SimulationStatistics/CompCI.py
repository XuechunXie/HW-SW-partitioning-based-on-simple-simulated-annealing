...
def main():
    ...
    i = getIdxofJmpInstList(l)

    #print(i)


    r = scan1(LabelList,l,i)
    ...
    #print(firstLineList)
    outputList = ["1 1 2 1", "2 2 3 1",
                  "3 3 4 1",
                  "4 4 5 1", "5 5 6 1"]

    firstLineList = giveFirstLineListCI(outputList)
    ...
    writeFileforCL('pptExampleCI.txt',firstLineList,outputList)


'''
    outputList = []
    for idx,partToLineListEachEdge in enumerate(r):
        outputList.append(giveOutputforEachEdge(idx,partToLineListEachEdge))
...
def scan(labelList,lastInstList,idxofJmpInstList):
    ...
    print(labelList)
    print(lastInstList)
    print(idxofJmpInstList)

    
    while (1):
        
        label_1 = labelList[scanIndex]
        lastInst_1 = lastInstList[scanIndex]
        idxJmp_1 = idxofJmpInstList[scanIndex]

        if (idxJmp_1 == 1):

            #get the label
            jmpLabel = lastInstList[scanIndex][4:-1]
            for i,item in enumerate(labelList):
                if (jmpLabel == item):
                    ...
                    label_2 = labelList[scanIndex]
                    lastInst_2 = lastInstList[scanIndex]
                    idxJmp_2 = idxofJmpInstList[scanIndex]
                    
                    countCI = 1

                    rowList.append(numOfEdge)
                    rowList.append(label_1)
                    rowList.append(label_2)
                    rowList.append(countCI)
                    
                    scanList.append(rowList)
                    rowList = []
...
def scan1(labelList,lastInstList,idxofJmpInstList):
...
def getNumOfTotalEdge(outputList):

    return len(outputList)

def getTotalCI(outputList):

    totalCL = 0

    for line in outputList:

        totalCL = totalCL + int(line[-1])

    return totalCL

def giveFirstLineListCI(outputList):

    firstList = []

    firstList.append(getNumOfTotalEdge(outputList))
    firstList.append(getTotalCI(outputList))
...
'''
def giveOutputforEachEdge(EdgeNumber,partToLineListEachEdge):

    
    resultListEachEdge = []
    resultListEachEdge.append(EdgeNumber)

    clEachEdge = 0
    
    for item in partToLineListEachEdge:
        clEachInst = calClForEachInst(item)
        clEachEdge = clEachEdge + int(clEachInst)

    
    resultListEachEdge.append(clEachEdge)



    instList = {}
    for eachline in partToLineListEachEdge:
    ...
    resultListEachEdge.append(nr_op)

    nr_kind_op = calNr_kind_op(instList)
    
    resultListEachEdge.append(nr_kind_op)

    

    return resultListEachEdge
'''
...
def getLabelList(listWithoutDivideToPartList):

    labelList = []

    for part in listWithoutDivideToPartList:
...
def getLastInstList(listWithoutDivideToPartList):
    ...
    for part in listWithoutDivideToPartList:
        ...
        #part[-1].split()
        lastInstList.append(part[-1])
        #print(part[-1])
    #print(lastInstList)
    return lastInstList


def getIdxofJmpInstList(lastInstList):

    #print(lastInstList)

    idxofJmpInstList = []

    for item in lastInstList:
        #print(item)
        lineList = item.split()
        #print(lineList)
        if (lineList[0] == "jmp"):
            idxofJmpInstList.append("1")
        else:
            idxofJmpInstList.append("0")



    
    return idxofJmpInstList


def writeFileforCI(myFileName,firstLineList,otherList):
...
def calCI(listWithoutDivideToPartList)
